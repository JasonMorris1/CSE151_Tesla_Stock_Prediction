# Introduction 
The stock market bussines is not unknown to common man. It was started in the early 1600's and has progessed significantly since then.  With the growing increase in making informed financial decisions a lot of people now choose to invest in stock markets. Stock prices are dynamic and hence depend on a multitude of factors. The growing popularity and demand of branded companies such as Tesla make predicting its stock price a very advantageous tool in the near future. It will not only benefit firms make market hypothesis but also help induviduals make financial decisions. We chose this project because we wanted to use the Machine Learning concepts taught in class to create an impact in the real world. Our aim with this project was to build a binary prediction Machine Learning model that could predict if the Tesla stock price would go up or down in the future. This model is important as it would allow an induvidual to study the Tesla Stock Price which would help them analyze the stock market better. Having a good predictive model would benefit the financial markets in creating effective hedging statergies and reducing potential losses. It would also reduce risk while investing by allowing induviduals to create an imminent balance between risk and return. We feel that this model could be used to research and predict the economic conditions of the market in the future. Researchers can draw analogies between the times when the model shows growth/decay in the stocks and the economy during that time to predict how similar situations in the future are going to affect the market. It also helps in ensuring economic stability by warning against potential market disruptions. This intersection between finance and technology not only helps in understanding the market better but also creates space for new innovative techniques to emerge which can help the economy grow.
