# Introduction 
The stock market bussines is not unknown to common man. It was started in the early 1600's and has progessed a lot since then. With the growing increase in making informed financial decisions a lot of people have chosen to spend their money accurately by investing it in stock markets. With the growing popularity and demand of branded companies such as Tesla, predicting its stock price can be a very advantageous tool in the near future. It will not only benefit firms make market hypothesis but also help induviduals make financial decisions. We though it would be cool to make a model that can help predict the next day closing price for a tesla stock. Although it would be impossible to come up with a very accurate model considering our dataset consists of time series data that is being updated every second, we have made an effort at applying the Machine Learning concepts learned in this class to make a forecast. Having a good predictive model would benefit the financial markets in creating effective hedging statergies and reducing potential losses. 

We chose this project because we wanted to use the Machine Learning concepts taught in class to create an impact in the real world. Stock prices are dynamic and hence depend on a multitude of factors. We felt that building a model that could consider all of these parameters and compellingly predict an output about its nature would help the investors and the economy grow. Our aim with this project was to build a binary prediction Machine Learning model that could predict if the Tesla stock price would go up or down in the future. The importance of this model is that it efficiently allows one to study the market well. It allows one to detect the pattern of the stock and then helps to decide if a particular investment would be helpful or not. It also reduces the risk one might face while investing, allowing them to create an imminent balance between risk and return. We also felt that this model could be used to research and predict the economic conditions of the market in the future. Using the growth/decay pattern of the stocks depicted by the model, researchers can draw analogies between the times when the model showed growth/decay and the economy during that time to predict how similar situations in the future are going to affect the market. It also helps in ensuring economic stability by warning against potential market disruptions. This intersection between finance and technology also helps to study the market better and creates space for new innovative techniques to emerge in order to help the economy grow.
